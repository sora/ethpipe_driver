...
static ssize_t ep_read(struct file *file, char __user *buf, size_t count,
				loff_t *ppos)
{
	unsigned int sender_wr_ptr_tmp = 0, sender_rd_ptr_tmp = 0;
...
loopx:
	...
	// mmio1 free space
	if (sender_rd_ptr_tmp <= sender_wr_ptr_tmp) {
		tx_free = (mmio1_len >> 1) - (sender_wr_ptr_tmp - sender_rd_ptr_tmp);
	} else {
	...
	if ( wait_event_interruptible( write_q, (tx_free > (mmio1_len >> 3)) ) ) {
		pr_info( "sender_rd_ptr_tmp: %d\n", (int)sender_rd_ptr_tmp );
		pr_info( "sender_wr_ptr_tmp: %d\n", (int)sender_wr_ptr_tmp );
		pr_info( "sender_rd_ptr: %d\n", *sender_rd_ptr );
		pr_info( "sender_wr_ptr: %d\n", *sender_wr_ptr );
		pr_info( "mmio1_len: %d\n", (int)mmio1_len );
		pr_info( "tx_free: %d\n", tx_free );
	...
	if ( (sender_wr_ptr_tmp + pktlen) > (mmio1_len >> 1) ) {
		...
		memcpy(mmio1_ptr + sender_wr_ptr_tmp, pkt, piece_len);
		memcpy(mmio1_ptr, (pkt + piece_len), (pktlen - piece_len));
	} else {
